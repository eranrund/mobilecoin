...
pub fn bootstrap_ledger(
    ...
    // Create the DB
    std::fs::create_dir_all(path.to_path_buf()).expect("Could not create ledger dir");
    LedgerDB::create(path.to_path_buf()).expect("Could not create ledger_db");
    let mut db = LedgerDB::open(path.to_path_buf()).expect("Could not open ledger_db");
